...
koaApp.use(async (ctx, next) => {
    console.log('我会读取一次')
    const template = serverCompiler.outputFileSystem.readFileSync(process.cwd() + '/dist/index.ssr.html', 'utf-8')
    //bundle是一个js或者json的绝对路径字符串,或者一个js对象，这里需要将读取到的转为对象
    const bundle = JSON.parse(serverCompiler.outputFileSystem.readFileSync(process.cwd() + '/dist/vue-ssr-server-bundle.json', 'utf-8'))
    ...
    await renderer.renderToString(context).then(html => {
        ...
        if (err.code === 404) {
            ...
            console.log('error in port :' + err.port)
            ctx.body = '服务器错误'
        }
    })
})
